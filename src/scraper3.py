...
def guardar_en_base_de_datos(juegos):
    if not juegos["current"] and not juegos["upcoming"]:
        print("⚠️ No hay juegos para guardar. Cancelando escritura en BD.")
        return
    ...
    # CORRECCIÓN: Tablas nombradas tal cual las busca check_db.py
    cursor.execute('''
    ...
    for juego in juegos["current"]:
        cursor.execute('''
            INSERT OR REPLACE INTO current_games (id, title, url, image_url, promo_type, end_date)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (juego["id"], juego["title"], juego["url"], juego["image_url"], juego["promo_type"], juego["end_date"]))
        
    # Guardar próximos juegos
    for juego in juegos["upcoming"]:
        cursor.execute('''
            INSERT OR REPLACE INTO upcoming_games (id, title, estimated_date, url)
            VALUES (?, ?, ?, ?)
        ''', (juego["id"], juego["title"], juego["estimated_date"], juego["url"]))
        
    conn.commit()
    conn.close()
    print("💾 Base de datos 'data/games.db' generada y actualizada con éxito.")
...
if __name__ == "__main__":
    ...
    guardar_en_base_de_datos(juegos)